"""
OrcaMax Code — Application Window
...
class OrcaMaxHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_skills_import_api(self):
        ...
        The API key is used ONLY for this single call — it is never persisted.
    ...
    def _check_agent_auth(self, body, allow_token_in_body=True):
        """Returns (ok, error_dict). By default OrcaMax has no auth — caller
        ...
        if not check_auth(token):
            return False, {"error": "unauthorized",
                           "hint": "auth is enforced — set HERMES_AGENT_TOKEN and HERMES_AGENT_AUTH=1 to enable"}
        return True, None

    def handle_agent_exec(self):
        """POST /api/agent/exec — single op or batch."""
        try:
            try:
            ...
            # Always 200 — failures are inside payload {ok:false, error:...}
            self.send_response(200)
    ...
    def handle_agent_ops(self):
        """GET /api/agent/ops — list all registered ops (no auth needed for discovery)."""
        try:
    ...
    def handle_agent_token(self):
        """GET /api/agent/token — disabled by default (OrcaMax is auth-free).
        ...
        try:
            ...
            if not _auth_enforced():
                self.json_response({
                    "ok": False,
                    "error": "auth disabled — OrcaMax is local-only, no token needed",
...
def main():
    ...
    # Start server
    with socketserver.TCPServer(("", PORT), OrcaMaxHandler) as httpd:
